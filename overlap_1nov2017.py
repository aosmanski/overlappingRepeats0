...
def get_hit_array(OUTPUT,rm_input,Iteration=0):
	if Iteration==0:
		lines2skip=3
	else:
		lines2skip=0
...
def main():
	...
	NEW_HIT_ARRAY=[]	
	HIT_ARRAY=get_hit_array(OUTPUT,rm_input,Iteration=0)

	while not IterateStop:
		#Call super bigfunction here. make sure all the variables are defined.

		NEW_HIT_ARRAY=superBIGfunction(HIT_ARRAY,OUTPUT)	
		if HIT_ARRAY==NEW_HIT_ARRAY:
			IterateStop=True
	
		else:
			HIT_ARRAY=NEW_HIT_ARRAY.copy()
			OUTPUT.close()
			arraynum+=1
			OUTPUT=open(LOC + '/' + OUT + str(arraynum),'w')
...
def superBIGfunction(HIT_ARRAY,OUTPUT):
	#iterator
	i = 0
...
#Repleace the stringbelow with "NEW_HIT_ARRAY.append(output_string)"
