...
def modRMhit(output_list):
	#SW_score, perc_div, perc_del, perc_ins, query_sequence, q_begin, q_end, q_left, orient, matching_repeat, class_family, r_begin, r_end, r_left, ID = output_list
...
def main():
	...
	while not IterateStop:
		...
		if HIT_ARRAY==NEW_HIT_ARRAY:
			IterateStop=True
	
		else:
			HIT_ARRAY=list(NEW_HIT_ARRAY)
			OUTPUT=open(LOC + '/' + OUT + '.' + str(arraynum) + '.out','w')
			# LIST COMPREHENSION: contig_occurrence = {x:0 for x in contig_list if x.startswith("contig")}
			NEW_HIT_ARRAY = [modRMhit(i) for i in NEW_HIT_ARRAY]
			OUTPUT.write("\n".join(NEW_HIT_ARRAY)+'\n')
			OUTPUT.close()
			arraynum+=1
			print("Iterations Complete: ",arraynum)
...
#			return
		...
		if CATEGORY=="CAT1":
			...
			if length_q>1:
				output_list=[SW_score, perc_div, perc_del, perc_ins, query_sequence, q_begin, q_end, q_left, orient, matching_repeat, class_family, r_begin, r_end, r_left, ID]
				
#Replace the string below with "NEW_HIT_ARRAY.append(output_list)"
				NEW_HIT_ARRAY.append(output_list)

			if length_q_i>1:
				output_list=[SW_score_i, perc_div_i, perc_del_i, perc_ins_i, query_sequence_i, q_begin_i, q_end_i, q_left_i, orient_i, matching_repeat_i, class_family_i, r_begin_i, r_end_i, r_left_i, ID_i]
				NEW_HIT_ARRAY.append(output_list)
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			...
			if perc_div <= perc_div_i:
				...
				output_list=[SW_score_i, perc_div_i, perc_del_i, perc_ins_i, query_sequence_i,
					q_begin_i1, q_end_i1, q_left_i, orient_i, matching_repeat_i, class_family_i, r_begin_i, r_end_i, r_left_i, ID_i+"L"]
				NEW_HIT_ARRAY.append(output_list)
				...
				output_list=[SW_score_i, perc_div_i, perc_del_i, perc_ins_i, query_sequence_i,
					q_begin_i2, q_end_i2, q_left_i, orient_i, matching_repeat_i, class_family_i, r_begin_i, r_end_i, r_left_i, ID_i+"R"]
...
#				OUTPUT.write(output_list)	
			...
			#Write top left portion	
				q_begin_1=q_begin
				q_end_1=q_begin_i-1
				output_list=[SW_score, perc_div, perc_del, perc_ins, query_sequence,
					q_begin_1, q_end_1, q_left, orient, matching_repeat, class_family, r_begin, r_end, r_left, ID+"L"]
#				OUTPUT.write(output_list)	
				NEW_HIT_ARRAY.append(output_list)
			...
			#Write top right portion
				...
				q_end_2=q_end
				output_list=[SW_score, perc_div, perc_del, perc_ins, query_sequence,
					q_begin_2, q_end_2, q_left, orient, matching_repeat, class_family, r_begin, r_end, r_left, ID+"R"]
#				OUTPUT.write(output_list)			
				NEW_HIT_ARRAY.append(output_list)
